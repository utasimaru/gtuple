# Githubにコード載せてます
https://github.com/utasimaru/gtuple/tree/main
似たようなことができるクレートがあるのですが、使い方がよくわかりませんでした。
だからシンプルな展開後コードを設計して、ジェミニにマクロを作ってもらいました。
gtupleと名付けたのですが、gにはジェネリックだったり、グループだったりという意味を込めています。
以下はReadmeと同じです。
...
戻り値があるメソッド：Ret $\rightarrow$ [Ret; N]（配列として一括取得）戻り値がないメソッド（() または省略）：配列化せず、各要素に対して順番に実行（セミコロン区切りで順次呼び出し）
